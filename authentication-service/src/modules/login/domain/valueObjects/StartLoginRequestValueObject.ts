...
class StartLoginRequestValueObject {
    ...
    emailIsString(): void {
        if (typeof this.email !== 'string')
            throw new Error(`StartLoginRequestValueObject: Invalid type provided for email property. Expected string, got ${typeof this.email}`)
    }

    emailLengthIsValid(){
        if (this.email.length < 1)
            throw new Error(`StartLoginRequestValueObject: email property could not be shorter than 1 character.`)

        if (this.email.length > 255)
            throw new Error(`StartLoginRequestValueObject: email property could not be larger than 255 characters.`)
    }

    async emailFormatIsValid() {
        const validationResult = await this.emailValidatorRepository.hasValidFormat(this.email)
        if (validationResult === false)
            throw new Error(`StartLoginRequestValueObject: email format is not valid`)
    }
