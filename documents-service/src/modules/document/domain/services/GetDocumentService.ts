...
class GetDocumentService {
  ...
  public async execute ({
    ...
    if (!hasPermission) { throw new Error('GetDocumentService: insufficient permissions to perform this operation') }

    const getDocumentResult = await this.documentRepository.get(documentEntity)

    if (getDocumentResult === null) { throw new Error('GetDocumentService: error while getting a document') }
